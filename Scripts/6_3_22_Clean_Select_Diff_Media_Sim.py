...
'''Right now, control which bacteria and plate are selected are by commenting and uncommenting 
global variables. Eventually these variables will be matched with the forms that come the 
webserver backend. Plate options and bacterial outcomes are controlled using the bac stat Dictionaries. Follow
the existing format within the dictionaries to add new plate options. If new proposed plates have 
special features, follow the model of "addBile" function for adding special features to the existing plot.
When changing core image generation parameters, use plate it up function, all key features of images
generated come from there or from bac stat libraries. When using program, update "plt.savefig()" command with 
appropritae address for svg file generation. MatplotLib figure viewer pop-up is scalable and thus images
can be altered based on window size, .svg file is fixed scale, so it is the better option for troubleshooting.
Hope this helps, contact Luke Meyers at lqmeyers@gmail.com for any questions or help with all image 
...
##-------------Official Names for fig Legend------------------

def getOfficialNames(plateIn,bacIn):
    '''returns official names to input into fig legend'''
    officialOut = []
    if plateIn == 'mac':
       officialOut.append('MacKonkey')
       officialOut.append(getBacName(bacIn))
    elif plateIn == 'msa':
       officialOut.append('Mannitol Salt')
       officialOut.append(getBacName(bacIn))
    elif plateIn == 'nutrient':
       officialOut.append('Nutrient')
       officialOut.append(getBacName(bacIn))
    elif plateIn == 'hek':  
       officialOut.append('Hektoen Enteric')
       officialOut.append(getBacName(bacIn))
    return officialOut

def getBacName(bacIn):
    '''returns offical name of bacIn as String'''
    if bacIn == 'staphEp':
        return('Staphulocaucus Epidermidus')
    elif bacIn == 'staphAur':
        return('Staphulocaucus Aureus')
    elif bacIn == 'eColi':
        return('Escheiria Coli')
    elif bacIn == 'pseudomonas':
        return('Pseudomona aeruginosa')
    elif bacIn =='salmonella':
...
mac = {'staphEp':[0,'#EBB503'],
            'staphAur':[0,'#EBB503'], 
            'eColi': [1,'#E36C7A','#E4A7B9',[50,50],'#EBB503'],
            'pseudomonas':[1,'#E36C7A','#F4F2E6',[50,50],'#EBB503'],
            'salmonella':[1,'#EBB503', '#FDD999',[50,50],'#EBB503'],
            'shigella': [1,'#EBB503','#E9E0E0',[50,50],'#EBB503']}
...
msa = {'staphEp':[1,'#E90E0E','#E36C7A',[45,50],'#E90E0E'],
           'staphAur':[1, '#FBC515', '#F0D783',[45,50],'#E90E0E'],
           'eColi': [0,'#E90E0E'],
...
hek = {'staphEp':[0,'#14707F'],
       'staphAur':[0,'#14707F'],
       'eColi':[1, '#FF4E00', '#FFDB00',[50,50],'#14707F'],
        #plate is blue but ecoli turn plate red/orange color due to bile production
       'pseudomonas':[1,'#14707F', '#EDF7E3',[50,50],'#14707F'],
       #check with Dr. R on pseudomonas for this media
       'salmonella':[1,'#14707F','#000000',[50,50],'#14707F'],
       'shigella':[1,'#14707F','#97CF5A',[50,50],'#14707F']}
...
def convexHull(p):
    ...
    p = np.array(p)
    hull = []#ConvexHull(p) #uses Scipy hull function. pay attention to capitalization
...
def getTanPolyPoints(centerList,r,mode='ccw'):
    '''gets the points for a polygon tangeantal to the circles at centerlist with r'''
    points = []  
...
'''ok so halo's are circle's centered around colonies,
blobs are polygons that connect circles tangeantaly (need to improve this feature)
to create hulls around the colonies. Convex hull rounds out all the sides using the halo's
but when using ccw sort, need to add a smoothing hull for concave vertices '''
...
def addBoth(plotName,mask,xData,yData,sIn,c,sMult,a=1,cutoff=4.6):
    ...
    for i in sIn:
        sOut.append(i*sMult)
    r = -2.52+.583*math.log(sOut[0]) #this formula still needs to be adjusted !!!!!!!!!! Used non-linear regression from guess and check proven values
...
def addBile(plotName,mask,xData,yData,s):
    ...
    '''ok so this feature is slightly unfinished, but decided to step away
    due to the Hek bile's kinda irrelevance to the educational purpose.
    I used a hex color gradiant generator to pick colors that were accurate, 
    becuase I couldnt use partial alpha colors as overlap would show.
    I left in a lot of commented color options for future improvement of bile 
    depiction '''
...
def addRealisticPlate(plotName,center=[0,0],r=10):
    ...
    '''ok a future direction is changing all of the integrers/floats in this section to be proportions
    of the radius, becuase then it can be applied to other versions of plate generators such as the 
    kiry baurer test. Would require signifigant troublshooting again, as I do not beleive that the 
    coordinate grid to matplotlib size attribute converstion is a linear relationship'''
...
def addControlPlate(plotName,gelColor,center,r=10):
    ...
    #--Show growth or not
    circle1 = plt.Circle((x,y),r,facecolor=gelColor,alpha=.55,edgecolor='white',linewidth = 3,)  # basic media circle
    plotName.add_artist(circle1)
    plotName.set_xlim(-12,36)
    addRealisticPlate(plotName,[x,y],r)  

#--------------Adding a figure legend to the generator to be embedded in the website---------------------------

officialName = getOfficialNames(plateIn,bacIn)

txt = officialName[0]+ ''' Agar inoculated with '''+officialName[1]+''' (left), negative control (right). 
...
plate color (listIn[1]), colony color (listIn[2]), colony size range (listIn[3]), and plate color
pre growth (listin[4]).
'''

def plateItUp(listIn,controlPlate=True,legend=True):
    ...
    if listIn[0] == 1:
        ...
        #--------testing scaling-------- #needed to find conversion!!
    ...
    if controlPlate == True:
        if listIn[0] == 0:
            ...
            addControlPlate(ax,listIn[4],[24,0],10)
    if legend == True:
        plt.subplots_adjust(bottom=.275)
        plt.figtext(.1,.1,txt,wrap=True,horizontalalignment='left',size=8,color='w')
    
    
###---------testing vars go in here---------------------------------------------------

plateItUp(getBacStats(plateIn,bacIn),True,True)
